...
const chartSeries = computed(() => {
  const overallSeries = {name:'总体平均',type:'bar',data:calculateDistribution(props.mergedData).satisfaction,itemStyle:{color:'#409EFF',opacity:.7},barGap:'20%'};
  ...
  const selectedSeries = props.selectedImpactNames.filter(name => name !== undefined && name !== null).map(name => {
    ...
    return {name: name,type:'bar',data:distributions.satisfaction,itemStyle:{color:props.config.COVID_IMPACT_MAP[impact].color,opacity:1}};
  }).filter(Boolean);
...
onMounted(() => {
  ...
  chartInstance.setOption({
    ...
    xAxis:{type:'category',data:props.config.SATISFACTION_BINS},
